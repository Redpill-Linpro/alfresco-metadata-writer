...
public class ThreadPoolExecutorFactoryBean implements FactoryBean<ExecutorService>, InitializingBean, DisposableBean {
  ...
  private static final RejectedExecutionHandler DEFAULT_REJECTED_EXECUTION_HANDLER = new TimeoutThreadPoolExecutor.CallerRunsPolicy();
  ...
  private TimeoutThreadPoolExecutor instance;
  ...
  /**
   * The priority that all threads must have on the scale of 1 to 10, where 1
   * has the lowest priority and 10 has the highest priority.
   * 
  ...
  /**
   * The maximum number of queued work instances to keep before blocking against
   * further adds.
   * 
   * @param size
   *          the queue size before blocks, or <b>-1</b> default to indicate no
   *          upper bound
   */
  ...
  /**
   * The optional handler for when tasks cannot be submitted to the queue. The
   * default is the <code>CallerRunsPolicy</code>.
   * 
  ...
  public void afterPropertiesSet() throws Exception {
    // if the maximum pool size has not been set, change it to match the core
    // pool size
    ...
    instance = new TimeoutThreadPoolExecutor(corePoolSize, maximumPoolSize, keepAliveTime, TimeUnit.SECONDS, workQueue, threadFactory, rejectedExecutionHandler, timeout, TimeUnit.SECONDS);
  }
